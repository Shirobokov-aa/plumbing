...
export default function Main() {
  ...
  // Проверьте, что sections не undefined/null перед использованием
  if (!sections) {
    console.log('Sections data is not loaded');
    return <div>Loading...</div>;
  }

  const imagesForCollections = useMemo(() => {
    if (!sections?.section4?.images_block) {
      return [];
    }
    return sections.section4.images_block.map((item) => ({
      src: item.src || "",
      alt: item.alt || "",
      desc: item.desc || "",
      url: item.url || "",
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="flex xl:flex-row flex-col lg:gap-20 gap-10">
            <div className="flex flex-col justify-between">
              <div className="max-w-[520px] flex flex-col gap-14">
                <h2 className="lg:text-h2 text-h2Lg">{sections.section1?.title}</h2>
                <p className="text-desc">{sections.section1?.description}</p>
              </div>
              <div className="flex xl:flex-col flex-col pt-5 gap-10">
                <div>
                  <Link
                    href={sections.section1?.link?.url || "#"}
                    className="text-desc border-b border-black"
                  >
                    {sections.section1?.link?.name}
                  </Link>
                </div>
                <div className="flex justify-between xl:gap-20 gap-5">
                  {sections.section1?.images_block?.map((item, index) => (
            ...
            <div className="">
              <div className="w-full h-full  flex justify-center items-center">
                <Image
                  src={sections.section1?.images?.[0] || "/img/fallback-image.png"}
                  alt=""
                  width={570}
                  height={500}
                  className="object-contain"
      ...
      <section>
        <div className="lg:block hidden max-w-1440 mx-auto lg:px-24 px-5 pt-24">
          <div className="relative w-full xl:h-[780px]">
            <Image
              src={sections.section2?.images?.[0] || "/img/fallback-image.png"}
              alt=""
              width={1240}
              height={780}
              className="object-contain "
            />
            <Link href={sections.section2?.link?.url || "#"} className="">
        ...
        <div className="max-w-1440 mx-auto lg:px-24 px-5 pt-24">
          <div className="flex xl:flex-row flex-col-reverse xl:gap-24 gap-5">
            <div className="xl:max-w-[526px] w-full">
              <Image
                src={sections.section3?.images?.[0] || "/img/fallback-image.png"}
                alt=""
                width={526}
                height={526}
                className="object-contain "
            ...
            <div className="xl:max-w-[614px] w-full flex flex-col justify-between">
              <div className="flex flex-col gap-11">
                <h2 className="lg:text-h2 text-h2Lg">{sections.section3?.title}</h2>
                <p className="lg:text-desc">{sections.section3?.description}</p>
              </div>
              <div className="xl:pt-0 pt-10">
                <Link href={sections.section3?.link?.url || "#"} className="text-desc border-b border-black">
                  {sections.section3?.link?.name}
                </Link>
              </div>
            </div>
          </div>
      ...
      <section>
        <div className="max-w-1440 mx-auto lg:px-24 px-5 pt-24">
          <div>
            <h2 className="lg:text-h2 text-h2Lg">{sections.section4?.title}</h2>
            <p className="lg:text-desc py-7">{sections.section4?.description}</p>
          </div>
          <div>
          ...
          <div className="flex justify-center items-center pt-16">
            <Link
              href={sections.section4?.link?.url || "#"}
      ...
      <section>
        <div className="max-w-1440 mx-auto lg:px-24 px-5 pt-24">
          <div className="flex xl:flex-row flex-col-reverse gap-5">
            <ImageBlock images={sections.section5?.images_block ?? []} />
            <div className="xl:max-w-[400px] w-full flex flex-col justify-between">
              <div>
                <h2 className="lg:text-h2 text-h2Lg">{sections.section5?.title}</h2>
                <p className="lg:text-desc pt-[45px]">
                {sections.section5?.description}
                </p>
              </div>
              <div className="xl:pt-0 pt-11">
                <Link href={sections.section5?.link?.url || "#"} className="text-desc border-b">
                {sections.section5?.link?.name}
                </Link>
              </div>
            </div>
          </div>
